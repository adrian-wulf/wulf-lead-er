...
def test_ip_rate_limiter_unit():
    limiter = IPRateLimiter(cooldown_seconds=300.0)
    ...
    # 2nd request from same IP rejected within 300s
    allowed, retry_after = limiter.check("1.2.3.4")
    assert allowed is False
    assert 200 < retry_after <= 300.0
...
def test_rate_limit_enabled_by_default_5min():
    assert scan_rate_limiter.cooldown_seconds == 300.0
    scan_rate_limiter.reset()
    allowed, _ = scan_rate_limiter.check("10.20.30.40")
    assert allowed is True
    allowed2, rem = scan_rate_limiter.check("10.20.30.40")
    assert allowed2 is False
    assert 250 < rem <= 300.0
    scan_rate_limiter.reset()
...
def test_rate_limit_can_be_disabled(monkeypatch):
    ...
    limiter = IPRateLimiter(cooldown_seconds=300.0)
    assert limiter.is_disabled() is True
    allowed, _ = limiter.check("1.1.1.1")
    assert allowed is True
    allowed2, _ = limiter.check("1.1.1.1")
    assert allowed2 is True


def test_rate_limit_scan_start_5min_cooldown():
    ...
    try:
        ...
        with patch.object(scan_manager, "start_scan", new_callable=AsyncMock) as mock_start:
            ...
            assert "5 minut" in res2.json()["detail"]
